...
class dsnote_app : public QObject
{
    ...
    Q_PROPERTY (int active_lang_idx READ active_lang_idx NOTIFY active_lang_changed)
    Q_PROPERTY (QString active_lang READ active_lang NOTIFY active_lang_changed)
    Q_PROPERTY (QVariantList available_langs READ available_langs NOTIFY available_langs_changed)
...
private:
    ...
    struct task_type {
        ...
        inline void set(int id) { previous = current; current = id; }
        inline bool operator==(int id) const { return id > INVALID_TASK && (current == id || previous == id); }
    ...
    void set_intermediate_text(const QString &text, const QString &lang, int task);
    [[nodiscard]] int active_lang_idx() const;
    [[nodiscard]] inline QString active_lang() { return active_lang_value; }
    [[nodiscard]] inline QString intermediate_text() const { return intermediate_text_value; }
