...
stt_service::stt_service(QObject *parent) : QObject{parent}, stt_adaptor{this} {
    ...
    connect(&manager, &models_manager::models_changed, this,
            &stt_service::handle_models_changed);
    connect(&manager, &models_manager::busy_changed, this,
            &stt_service::refresh_status);
    connect(&manager, &models_manager::download_progress, this,
    ...
    connect(&single_sentence_task_timer, &QTimer::timeout, this,
            &stt_service::handle_task_timeout);
...
stt_service::source_type stt_service::audio_source_type() const {
    if (!source) return source_type::none;
...
std::optional<stt_service::model_files_type> stt_service::choose_model_files(
    ...
    for (const auto &model : models) {
        ...
        available_models_map.emplace(
            model.id, model_data_type{model.id, model.lang_id, model.name});
    }
...
void stt_service::handle_models_changed() {
    ...
    if (current_task && available_models_map.find(current_task->model_id) ==
                            available_models_map.end()) {
        stop();
    }
...
QString stt_service::restart_ds(speech_mode_type speech_mode,
                                const QString &model_id) {
    if (const auto model_files = choose_model_files(model_id)) {
        ...
        const auto speech_mode_val =
            static_cast<deepspeech_wrapper::speech_mode_type>(speech_mode);

        if (ds && ds->model_file() == model_file_str &&
            ds->scorer_file() == scorer_file_str) {
            qDebug() << "restart ds not required:"
                     << static_cast<int>(speech_mode_val);
            ...
            deepspeech_wrapper::callbacks_type call_backs{
                ...
                std::bind(
                    static_cast<void (stt_service::*)(const std::string &)>(
                        &stt_service::handle_intermediate_text_decoded),
                    this, std::placeholders::_1),
                std::bind(&stt_service::handle_speech_status_changed, this,
                          std::placeholders::_1)};

            ds = std::make_unique<deepspeech_wrapper>(
                model_file_str, scorer_file_str, call_backs, speech_mode_val);
        }
...
void stt_service::handle_intermediate_text_decoded(const QString &,
                                                   const QString &,
                                                   int task_id) {
    ...
    if (current_task) {
        if (previous_task && last_intermediate_text_task == previous_task->id) {
            emit text_decoded(QString::fromStdString(text),
                              previous_task->model_id, previous_task->id);
        } else {
            emit text_decoded(QString::fromStdString(text),
                              current_task->model_id, current_task->id);
        }
    } else {
        qWarning() << "current task does not exist";
    }
...
void stt_service::handle_speech_clear() {
    if (source && (state() == state_type::listening_auto ||
                   state() == state_type::transcribing_file)) {
        source->clear();
    }
}
...
void stt_service::set_progress(double p) {
    if (audio_source_type() == source_type::file && current_task) {
        ...
        if (delta > 0.01 || p < 0.0 || p >= 1) {
            progress_value = p;
            emit transcribe_file_progress_changed(progress_value,
                                                  current_task->id);
        }
    }
}
...
int stt_service::cancel_file(int task) {
    if (state() == state_type::unknown ||
        state() == state_type::not_configured || state() == state_type::busy) {
    ...
    if (audio_source_type() == source_type::file) {
        if (current_task && current_task->id == task) {
            if (pending_task) {
                ...
                if (current_task->speech_mode ==
                    speech_mode_type::single_sentence) {
                    single_sentence_task_timer.start();
                }
...
int stt_service::transcribe_file(const QString &file, const QString &lang) {
    ...
    current_task = {next_task_id(), speech_mode_type::automatic,
...
void stt_service::restart_audio_source(const QString &source_file) {
    if (ds) {
        ...
        set_progress(source->progress());
        connect(source.get(), &audio_source::audio_available, this,
                &stt_service::handle_audio_available, Qt::QueuedConnection);
        connect(source.get(), &audio_source::error, this,
                &stt_service::handle_audio_error, Qt::QueuedConnection);
        connect(source.get(), &audio_source::ended, this,
...
void stt_service::refresh_status() {
    ...
    if (new_state != state_value) {
        qDebug() << "state changed:" << state_str(state_value) << "=>"
                 << state_str(new_state);
...
QString stt_service::state_str(state_type state_value) {
    switch (state_value) {
        case state_type::busy:
            return QStringLiteral("busy");
        case state_type::idle:
            return QStringLiteral("idle");
        case state_type::listening_manual:
            return QStringLiteral("listening_manual");
        case state_type::listening_auto:
            return QStringLiteral("listening_auto");
        case state_type::not_configured:
            return QStringLiteral("not_configured");
        case state_type::transcribing_file:
            return QStringLiteral("transcribing_file");
        case state_type::listening_single_sentence:
